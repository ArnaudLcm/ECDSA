...
final class ECCPointTests: XCTestCase {
  ...
  func testPointAdditionWithInversePoints() throws {
    ...
    let b = FiniteElement(value: 7, prime: 223)
    let p1 = try ECCPoint(
      a: a, b: b, x: FiniteElement(value: 47, prime: 223), y: FiniteElement(value: 71, prime: 223))
    let p2 = try ECCPoint(
      a: a, b: b, x: FiniteElement(value: 47, prime: 223), y: FiniteElement(value: 152, prime: 223))  // Inverse of p1
  ...
  func testPointAdditionWithDistinctPoints() throws {
    ...
    let b = FiniteElement(value: 7, prime: 223)
    let p1 = try ECCPoint(
      a: a, b: b, x: FiniteElement(value: 170, prime: 223), y: FiniteElement(value: 142, prime: 223)
    )
    let p2 = try ECCPoint(
      a: a, b: b, x: FiniteElement(value: 60, prime: 223), y: FiniteElement(value: 139, prime: 223))

    let expected = try ECCPoint(
      a: a, b: b, x: FiniteElement(value: 220, prime: 223), y: FiniteElement(value: 181, prime: 223)
    )
  ...
  func testPointAdditionFailsForInvalidPoints() throws {
    ...
    XCTAssertThrowsError(
      try ECCPoint(
        a: a, b: b, x: FiniteElement(value: 1, prime: 223), y: FiniteElement(value: 1, prime: 223))
    ) { error in
      XCTAssertEqual(error as? ECDSAError, .invalidCurveDefinition("Point is not on the curve."))
    }
  }

  func testScalarMultiplicationByOne() throws {
    let a = FiniteElement(value: 0, prime: 223)
    let b = FiniteElement(value: 7, prime: 223)
    let p = try ECCPoint(
      a: a, b: b, x: FiniteElement(value: 192, prime: 223), y: FiniteElement(value: 105, prime: 223)
